...
async function fetchStockData() {
    ...
    try {
        const stockData = await Promise.all(tickersArr.map(async (ticker) => {
            const url = `https://polygon-api-worker.testapp123.workers.dev/?ticker=${ticker}&startDate=${dates.startDate}&endDate=${dates.endDate}`
...
async function fetchReport(data) {
    ...
    try {
        const url = 'https://openai-api-worker.testapp123.workers.dev'
        ...
        if (!response.ok) {
            ...
            console.error('Response Data:', data);               // Log the data returned from the server
            throw new Error(`Worker Error: ${data.error || 'Unknown error occurred'}`);
        }
